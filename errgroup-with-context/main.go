...
func main() {
	...
	var choice int
	fmt.Println("Choose length of time(in milliseconds)?")
	fmt.Print("Enter 1=200 2=1500:")
	...
	switch choice {
	case 1:
		dur = time.Duration(200)
	case 2:
		dur = time.Duration(1500)
	...
	for _, tc := range []tc{
		...
		log.Printf("\n%s\n%s\n%s\n", strings.Repeat("-", len(tc.name)), tc.name, strings.Repeat("-", len(tc.name)))

		ctx, cancel := context.WithTimeout(context.Background(), dur*time.Millisecond) // 1500*time.Millisecond)
		...
		log.Printf("finished in %s:\n\t%s", time.Since(start), jsonString(rsp))
